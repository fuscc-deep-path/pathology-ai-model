...
def Wfast(img, nstains, lamb, num_patches, patchsize, level, background_correction=False):
    ...
    if len(valid_inp) > 0:
        ...
        for i in range(len(valid_inp)):
            WS.append(getstainMat(valid_inp[i], param, i0))
        ...
        print('Wsource sort processing...')
        Wsource = W_sort(preprocessing.normalize(Wsource, axis=0, norm='l2'))
...
def BLtrans(Ivecd, i_0):
    print('BLtransforming...')
    s = Ivecd.shape
    third_dim = 1 if len(s) == 2 else s[2]
    Ivecd = np.reshape(Ivecd, (s[0] * s[1], third_dim))  # vertorize the Ivecd

    V = np.log(i_0) - np.log(Ivecd + 1.0)
    w_threshold = 220
    # c = (Ivecd[:, 0] < w_threshold) * (Ivecd[:, 1] < w_threshold) * (Ivecd[:, 2] < w_threshold)
    c = (np.mean(Ivecd, axis=1) < w_threshold)
    Ivecd = Ivecd[c]
